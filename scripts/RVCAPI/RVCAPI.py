...
def infer(pth_file: str, input_file: str, output_file: str, feature_index: str, speaker_id: int, transpoition: float, f0_method: str, crepe_hop: int, harvest_median_filter: int, post_sample_rate: int, mix_vol_envelope: float, feature_index_ratio: float, VCP: float) -> str:
    ...
    #### Parameters:
    pth_file ```str```: 
        .pth model name with (in lib/weights).
    input_file ```str```:
        The input file to convert. All major audio formats are supported
    output_file ```str```:
        The name of the output file (with .wav extension) to be placed in lib/audio-outputs
    feature_index ```str```:
        Path to feature .index file.
    speaker_id ```int```:
        Which speaker the new audio file should mimic. Majority of .pth files are trained on 1 speaker, such as in 3b1b use case. Default is ```0```
    transposition ```float```:
        How much to transpose (change the pitch) the audio. Higher values should be used if target's voice is higher pitched than input voice, and lower values should be used if target's voice is lower pitched than input voice. Default is ```0.0```
    f0 method ```str```:
        Method to use for pitch detection. Options are "pm", "crepe", "crepe-tiny", "mangio-crepe", "mangio-crep-tiny", "rmvpe", or multiple methods. For multiple methods, use the following format: "hybrid[method1+method2+method3]" etc. It's recommended to use whatever model is trained with, for 3b1b this is rmvpe. Default is ```rmvpe```
    crepe_hop ```int```:
        The hop size for crepe. Only applicable if atleast 1 chosen f0 method is a type of crepe. Default is ```160```
    harvest_median_filter ```int``` ```0-7```:
        The median filter radius size for harvest. Only applicable if atleast 1 chosen f0 method harvest. Min 0, max 7, ```default``` is 3.
    post_sample_rate ```int```:
        The resample rate for the output audio.  Default is ```0``` (meaning nothing will happen).
    mix_vol_envelope ```float```:
        The volume envelope of the output audio. Default is ```1.0```
    feature_index_ratio ```float``` ```0-1```:
        The ratio of the feature index to use. Default is ```1.0```
    VCP ```float```:

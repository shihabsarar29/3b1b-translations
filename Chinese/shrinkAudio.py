...
def adjust_audio(translated_file_path, json_file_path, output_file_path):
    ...
    translated_audio = AudioSegment.from_file(translated_file_path)
    ...
    if original_entry:
        ...
        time_range = original_entry.get("time_range", [0, len(translated_audio)])

        # Load the original English audio file based on the time range
        original_filename = original_entry.get("input", "")
        original_audio = AudioSegment.from_file(original_filename)[int(time_range[0] * 1000):int(time_range[1] * 1000)]
...
adjust_audio(translated_file_path, json_file_path, output_directory)
